...
def draw_sentence():
    ...
    title_surf, title_rect = make_text_objs(SENTENCE, BIGFONT, TEXTCOLOR)
    title_rect.center = (int(WINDOWWIDTH / 2) , int(WINDOWHEIGHT / 2) )
    DISPLAYSURF.blit(title_surf, title_rect)

    typed_text = SENTENCE[:SENT_INDEX]
    typed_surf, typed_rect = make_text_objs(typed_text, BIGFONT, TEXTHIGHLIGHT)
    ...
    next_surf, next_rect = make_text_objs(SENTENCE[:SENT_INDEX + 1], BIGFONT, TEXTHIGHLIGHT)
